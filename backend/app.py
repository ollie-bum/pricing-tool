...
, but the browser might not have the necessary session cookie, or the session has expired, leading to a redirect to `/login`.
  - The redirect response (HTML) is then misinterpreted as JSON by `script.js`.

#### Why This Happens
- The `fetchAvailableModels` call happens immediately when the page loads (`DOMContentLoaded` event), but the browser might not have the session cookie set properly due to:
  1. **Session Cookie Domain**: The session cookie might not be shared across subdomains (`pricingtool.maisonbum.com` vs. `pricingtoolbulk.maisonbum.com`), or it might not be set correctly after login.
  2. **Timing Issue**: The `fetch` request might be sent before the browser fully processes the login session.

### Solution
Let’s fix this by ensuring the session cookie is shared across subdomains and that the `/api/models` request only proceeds if the user is authenticated. We’ll also add a check in `script.js` to handle the redirect more gracefully.

#### Step 1: Configure Session Cookie for Subdomains
Set the `SESSION_COOKIE_DOMAIN` in `app.py` to `.maisonbum.com` to ensure the session cookie is shared across subdomains.

<xaiArtifact artifact_id="d27a3b0b-8228-48ea-8b6d-961a74d13890" artifact_version_id="90422e8b-a2ae-4bf1-a163-0f71ff2fc43f" title="app.py" contentType="text/python">
# backend/app.py
try:
    from werkzeug.urls import url_quote
except ImportError:
